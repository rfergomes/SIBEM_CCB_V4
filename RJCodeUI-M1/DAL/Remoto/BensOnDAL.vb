...
Public Class BensOnDAL
    Inherits GenericDAL(Of BensOnDTO)

    Private Tabela = "bens"
    Private TabelaViews = "bens"

    Public Sub New(connectionFactory As ConnectionFactory)
        MyBase.New(connectionFactory)
    End Sub

    Public Overrides Function Insert(bens As BensOnDTO) As Integer
    ...
    Public Overrides Function Update(bens As BensOnDTO) As Integer
        Dim columns As New Dictionary(Of String, Object)()
        columns.Add("id_bem", bens.Id)
        columns.Add("descricao", bens.Descricao)
        columns.Add("id_igreja", bens.Id_igreja)
    ...
    Public Overrides Function GetByPart(part As String) As BensOnDTO
        Dim columns As New Dictionary(Of String, Object)()
        Dim lista As List(Of BensOnDTO) = BuscarLista(TabelaViews, columns, $"AND descricao = '{part}' LIMIT 1")
    ...
    Protected Overrides Function ConvertFromReader(reader As Common.DbDataReader) As BensOnDTO
        Dim bens As New BensOnDTO()
        ...
        If ColumnExists(reader, "id_dependencia") AndAlso Not reader.IsDBNull(reader.GetOrdinal("id_dependencia")) Then
            If reader.GetFieldType(reader.GetOrdinal("id_dependencia")) = GetType(Int32) Then
                bens.Id_Dependencia = reader.GetInt32(reader.GetOrdinal("id_dependencia"))
            End If
        End If
        ...
        If ColumnExists(reader, "id_setor") AndAlso Not reader.IsDBNull(reader.GetOrdinal("id_setor")) Then
            If reader.GetFieldType(reader.GetOrdinal("id_setor")) = GetType(String) Then
                bens.Id_Setor = reader.GetString(reader.GetOrdinal("id_setor"))
            End If
        End If
        ...
        If ColumnExists(reader, "setor") AndAlso Not reader.IsDBNull(reader.GetOrdinal("setor")) Then
            If reader.GetFieldType(reader.GetOrdinal("setor")) = GetType(String) Then
                bens.Setor = reader.GetString(reader.GetOrdinal("setor"))
            End If
        End If
        ...
        If ColumnExists(reader, "id_status") AndAlso Not reader.IsDBNull(reader.GetOrdinal("id_status")) Then
            If reader.GetFieldType(reader.GetOrdinal("id_status")) = GetType(Int32) Then
                bens.Id_Status = reader.GetInt32(reader.GetOrdinal("id_status"))
            End If
        End If
